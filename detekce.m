function [ SCORE ] = detekce(im,im_maska)
...
% s=zeros(size(im_bin));
% 
% p(im_bin==1 & im_maska(1:size(im,1)-5,1:size(im,2)-5)==1)=1;
% s(im_bin==1 | im_maska(1:size(im,1)-5,1:size(im,2)-5)==1)=1;
% 
% score = sum(sum(p))/sum(sum(s))
